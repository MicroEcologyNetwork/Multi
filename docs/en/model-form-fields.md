...
})->ajax('/multi/api/users');

// using ajax and show selected item:

$form->select('user_id')->options(User::class)->ajax('/multi/api/users');

// or specifying the name and id

$form->select('user_id')->options(User::class, 'name', 'id')->ajax('/multi/api/users');
```

<sub>Notice：if you have modified the value of the `route.prefix` in the `config/multi.php` file, this api route should be modified to `config('multi.route.prefix').'/api/users'`.</sub>

The controller method for api `/multi/api/users` is:
...
The json returned from api `/multi/demo/options`:
```
{
    "total": 4,
    "per_page": 15,
...
You can use compression, crop, add watermarks and other methods, please refer to [[Intervention] (http://image.intervention.io/getting_started/introduction)], picture upload directory in the file `config / multi.php` `Upload.image` configuration, if the directory does not exist, you need to create the directory and open write permissions:
```php
...
The file upload directory is configured in `upload.file` in the file `config/multi.php`. If the directory does not exist, it needs to be created and write-enabled.
```php
