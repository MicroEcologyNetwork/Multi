...
(function ($) {
    ...
    $.multi.reload = function () {
        $.pjax.reload('#pjax-container');
        $.multi.grid = new Grid();
    };

    $.multi.redirect = function (url) {
        $.pjax({container:'#pjax-container', url: url });
        $.multi.grid = new Grid();
    };

    $.multi.getToken = function () {
        return $('meta[name="csrf-token"]').attr('content');
    };

    $.multi.loadedScripts = [];

    $.multi.loadScripts = function(arr) {
        var _arr = $.map(arr, function(src) {

            if ($.inArray(src, $.multi.loadedScripts)) {
                return;
            }

            $.multi.loadedScripts.push(src);
