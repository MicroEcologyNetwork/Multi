...
<sub>注：如果你修改了`config/multi.php`配置文件中`route.prefix`的值，此处的接口路由应该修改为`config('multi.route.prefix').'/api/users'`。</sub>

API `/multi/api/users`接口的代码：
...
图片上传目录在文件`config/multi.php`中的`upload.image`中配置，如果目录不存在，需要创建该目录并开放写权限。
...
文件上传目录在文件`config/multi.php`中的`upload.file`中配置，如果目录不存在，需要创建该目录并开放写权限。
```php
