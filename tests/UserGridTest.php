...
class UserGridTest extends TestCase
{
    ...
    public function testIndexPage()
    {
        ...
        $urlAll = url('/multi/users?_export_=all');
        $urlNew = url('/multi/users/create');
    ...
    public function testGridPagination()
    {
        ...
        $this->visit('multi/users?page=2');
        $this->assertCount(20, $this->crawler()->filter('td a i[class*=fa-edit]'));

        $this->visit('multi/users?page=3');
        $this->assertCount(20, $this->crawler()->filter('td a i[class*=fa-edit]'));

        $this->visit('multi/users?page=4');
        $this->assertCount(5, $this->crawler()->filter('td a i[class*=fa-edit]'));

        $this->click(1)->seePageIs('multi/users?page=1');
        $this->assertCount(20, $this->crawler()->filter('td a i[class*=fa-edit]'));
    }
    ...
    public function testOrderByJson()
    {
        ...
        $this->visit('multi/users?_sort[column]=data.json.field&_sort[type]=desc&_sort[cast]=unsigned');
    ...
    public function testLikeFilter()
    {
        ...
        $this->visit('multi/users?username=mi');
    ...
    public function testFilterRelation()
    {
        ...
        $this->visit('multi/users?email='.$user->email)
            ->seeInElement('td', $user->username)
            ->seeInElement('td', $user->email)
            ->seeInElement('td', $user->mobile)
            ->seeElement("img[src='{$user->avatar}']")
    ...
    public function testGridPerPage()
    {
        ...
        $this->assertEquals('http://localhost:8000/multi/users?per_page=20', $this->crawler()->filter('select option[selected]')->attr('value'));

        $perPage = rand(1, 98);

        $this->visit('multi/users?per_page='.$perPage)
