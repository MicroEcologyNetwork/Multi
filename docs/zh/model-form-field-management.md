...
找到文件`app/Multi/bootstrap.php`,如果文件不存在，请更新`laravel-multi`，然后新建该文件
...
[wangEditor](http://www.wangeditor.com/)是一个优秀的国产的轻量级富文本编辑器，如果`laravel-multi`自带的基于`ckeditor`的编辑器组件使用上有问题，可以通过下面的步骤可以集成它，并覆盖掉`ckeditor`：

先下载前端库文件[wangEditor](https://github.com/wangfupeng1988/wangEditor/releases)，解压到目录`public/vendor/wangEditor-3.0.9`。

然后新建组件类`app/Multi/Extensions/WangEditor.php`。
...
{
    protected $view = 'multi.wang-editor';
...
新建视图文件`resources/views/multi/wang-editor.blade.php`：
```php
...
然后注册进`laravel-multi`,在`app/Multi/bootstrap.php`中添加以下代码：

```php

<?php

use App\Multi\Extensions\WangEditor;
use MicroEcology\Multi\Form;
...
然后新建扩展文件`app/Multi/Extensions/Form/CKEditor.php`:
```php
<?php
...
新建view `resources/views/multi/ckeditor.blade.php`:
```php
...
然后在`app/Multi/bootstrap.php`中引入扩展：
```php
...
新建组件类`app/Multi/Extensions/PHPEditor.php`:
...
>类中的静态资源也同样可以从外部引入，参考[Editor.php](https://github.com/z-song/laravel-multi/blob/1.3/src/Form/Field/Editor.php)

创建视图`resources/views/multi/php-editor.blade.php`:
...
最后找到文件`app/Multi/bootstrap.php`,如果文件不存在，请更新`laravel-multi`，然后新建该文件,添加下面代码：
