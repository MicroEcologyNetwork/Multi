@if($errors->hasBag('exception') && config('app.debug') == true)
    ...
    <div class="alert alert-warning alert-dismissable">
        ...
        <p><a style="cursor: pointer;" onclick="$('#laravel-multi-exception-trace').toggleClass('hidden');$('i', this).toggleClass('fa-angle-double-down fa-angle-double-up');"><i class="fa fa-angle-double-down"></i>&nbsp;&nbsp;{!! $error->first('message') !!}</a></p>

        <p class="hidden" id="laravel-multi-exception-trace"><br>{!! nl2br($error->first('trace')) !!}</p>
    </div>
@endif
