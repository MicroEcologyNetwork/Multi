        ...
        <div id="has-many-{{$column}}" style="margin-top: 15px;">
                ...
                @foreach($forms as $pk => $form)
                    <tr class="has-many-{{$column}}-form fields-group">
                        ...
                        @foreach($form->fields() as $field)
                            ...
                            @if (is_a($field, \MicroEcology\Multi\Form\Field\Hidden::class))
                        ...
                        @if($options['allowDelete'])
                            <td class="form-group">
                                <div>
                                    <div class="remove btn btn-warning btn-sm pull-right"><i class="fa fa-trash">&nbsp;</i>{{ trans('multi.remove') }}</div>
                                </div>
                            </td>
                        @endif
                    </tr>
            ...
            @if($options['allowCreate'])
                <div class="form-group">
                    <div class="{{$viewClass['field']}}">
                        <div class="add btn btn-success btn-sm"><i class="fa fa-save"></i>&nbsp;{{ trans('multi.new') }}</div>
                    </div>
                </div>
            @endif
        </div>
