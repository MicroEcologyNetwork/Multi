...
abstract class Extension
{
    ...
    public static function boot()
    {
        ...
        Multi::extend($extension->name, get_called_class());
    ...
    /**
     * Get config set in config/multi.php.
     *
     * @param string $key
     * @param null   $default
     *
    ...
    {
        $name = array_search(get_called_class(), Multi::$extensions);

        if (is_null($key)) {
            $key = sprintf('multi.extensions.%s', strtolower($name));
        } else {
            $key = sprintf('multi.extensions.%s.%s', strtolower($name), $key);
        }
    ...
    /**
     * Import menu item and permission to laravel-multi.
     */
    ...
    protected function getMenuValidationRules()
    {
        return [
            'title'    => 'required',
            'path'     => ['required', Rule::unique(Config::get('multi.database.menu_table'), 'uri')],
            'icon'     => 'required',
            'children' => 'nullable|array',
        ];
    }
    ...
    protected function getPermissionValidationRules()
    {
        return [
            'name'     => 'required',
            'slug'     => ['required', Rule::unique(Config::get('multi.database.permissions_table'), 'slug')],
            'path'     => 'required|array',
            'path.*'   => 'string',
    ...
    /**
     * Create a item in laravel-multi left side menu.
     *
     * @param string $title
     * @param string $uri
     * @param string $icon
