...
And then in `config/multi.php` select the `disk` set up above：
...
Set `disk` to the` multi` that you added above,`directory.image` and `directory.file` is the upload path for `$form->image($column)` and `$form->file($column)`.
...
Then modify the upload configuration of `laravel-multi` and open `config/multi.php` to find:
