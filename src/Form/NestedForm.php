...
class NestedForm
{
    ...
    /**
     * @var \MicroEcology\Multi\Form|\MicroEcology\Multi\Widgets\Form
     */
    protected $form;
    ...
    protected function prepareRecord($record)
    {
        ...
        foreach ($this->fields as $field) {
            ...
            if (($field instanceof \MicroEcology\Multi\Form\Field\Hidden) || $value != $field->original()) {
                if (is_array($columns)) {
    ...
    public function getTemplateHtmlAndScript()
    {
        ...
        foreach ($this->fields() as $field) {
            ...
            //when field render, will push $script to Multi
            $html .= $field->render();

            /*
             * Get and remove the last script of Multi::$script stack.
             */
            if ($field->getScript()) {
                $scripts[] = array_pop(Multi::$script);
            }
        }
