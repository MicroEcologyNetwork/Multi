...
abstract class Action implements Renderable
{
    ...
    {
        return multi_url('_handle_action_');
    }
...
var actionResolver = function (data) {
            ...
            if (typeof response !== 'object') {
                return $.multi.swal({type: 'error', title: 'Oops!'});
            }
            ...
            if (typeof response.swal === 'object') {
                $.multi.swal(response.swal);
            }

            if (typeof response.toastr === 'object' && response.toastr.type) {
                $.multi.toastr[response.toastr.type](response.toastr.content, '', response.toastr.options);
            }
        ...
        var actionCatcher = function (request) {
            if (request && typeof request.responseJSON === 'object') {
                $.multi.toastr.error(request.responseJSON.message, '', {positionClass:"toast-bottom-center", timeOut: 10000}).css("width","500px")
            }
        };
SCRIPT;

        Multi::script($resolve);
