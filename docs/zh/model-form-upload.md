...
然后选择上传的`disk`，打开`config/multi.php`找到：
...
将`disk`设置为上面添加的`multi`，`directory.image`和`directory.file`分别为用`$form->image($column)`和`$form->file($column)`上传的图片和文件的上传目录。
...
然后修改`laravel-multi`的上传配置，打开`config/multi.php`找到：
